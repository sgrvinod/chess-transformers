...
# Filter games that had a time control of at least 5 minutes and ended in a checkmate 
...
for f in split-*; do
    {
        out="${f}.pgn"       
        pgn-extract -f "$f" -t tags.txt --checkmate -s -o "$out"
    } &
done
wait
cat split-*.pgn > filtered_games.pgn
rm -- split-* all.txt tags.txt
